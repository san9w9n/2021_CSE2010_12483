...
int main()
{
	...
	//sDelete Data in Tree(pParentNode)
...
Node* searchTreeNode(Node* p, int value)
{
	...
	while (p != NULL && p->data != value)
	{
        ...
        else if (p->data > value) p = p->leftChild;
	}
	//찾았으면 해당 노드의 주소를 반환
	//못찾았으면 NULL을 반환
	return p;
...
void deleteTreeNode(Node** p, int value)
{
	...
	if ((delNode->leftChild==NULL) && (delNode->rightChild==NULL))
	{
		...
		else
		{
            ...
            if(parentNode->leftChild == delNode) parentNode->leftChild = NULL;
	...
	else if ((delNode->leftChild == NULL) || (delNode->rightChild==NULL))
	{
		...
		else
		{
            ...
            if (parentNode->leftChild == delNode) parentNode->leftChild = childNode;
	...
	//Case 3. 자식 노드가 둘 다 있는 경우
		...
		if ((succNode->leftChild==NULL) && (succNode->rightChild==NULL)) {
			if(succParent->leftChild == succNode) succParent->leftChild = NULL;
			else succParent->rightChild = NULL;
		}
		else {
			Node* succChild = succNode->rightChild;
			if (succParent->leftChild == succNode) succParent->leftChild = succChild;
			else succParent->rightChild = succChild;
		}

		if (parentNode == NULL) *p = succNode;
		else {
			if (parentNode->leftChild == delNode) parentNode->leftChild = succNode;
			else parentNode->rightChild = succNode;
		}
...
int compareTwoTree(Node* t1, Node* t2)
{
	if (t1 == NULL && t2 == NULL) return 1;
		...
		else if (!compareTwoTree(t1->leftChild, t2->leftChild)) return 0;
		else if (!compareTwoTree(t1->rightChild, t2->rightChild)) return 0;
		return 1;
	}
}
